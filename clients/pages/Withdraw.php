...
<body>
        ...
        <aside class="sidebar" id="sidebar">
            ...
            <nav class="nav-menu">
                ...
                <a href="Transfer.php" class="nav-link"><i class="fa-solid fa-money-bill-transfer"></i> Transfer
                    money</a>
                <a href="Withdraw.php" class="nav-link active"><i class="fa-solid fa-arrow-up-from-bracket"></i>
                    Withdraw</a>
                ...
                <?php if (!$success && !$pendingApproval): ?>
                    <div class="header-actions" style="margin-bottom: 32px;">
                        <div class="header-welcome">
                            <div class="date-text"><?= $current_date ?></div>
                            <h1 style="font-size: 28px; font-weight: 800; color: var(--text-main); margin: 0;">Withdraw
                                Money</h1>
                        </div>
                    </div>

                    <div class="alert alert-danger" id="withdrawError"
                        style="border-radius: 12px; margin-bottom: 24px; <?= $error ? '' : 'display:none;' ?>">
                    ...
                    <form method="POST" id="withdrawForm">
                        <div class="widget" style="padding: 32px; border-radius: 24px;">
                            <div class="form-group" style="margin-bottom: 24px;">
                                ...
                                <input type="text" name="card_num" placeholder="6-digit card number" class="form-control"
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 24px;">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="expire" placeholder="dd/mm/yyyy" class="form-control"
                                        style="width: 100%; padding: 14px; border-radius: 12px; border: 1px solid var(--border-color); background: var(--bg-surface); color: var(--text-main);"
                            ...
                            <div class="form-group" style="margin-bottom: 24px;">
                                ...
                                <input type="text" name="amount" id="amountInput" placeholder="Multiples of 50,000"
                                    class="form-control"
                                    style="width: 100%; padding: 14px; border-radius: 12px; border: 1px solid var(--border-color); background: var(--bg-surface); color: var(--text-main);"
                                    value="<?= $amount > 0 ? number_format($amount, 0, ',', '.') : '' ?>" required>
                                <div id="amountError"
                                    style="color: #ef4444; font-size: 12px; margin-top: 8px; display: none; align-items: center; gap: 6px;">
                            ...
                            <div id="feePreview" class="fee-preview"
                                style="background: rgba(255,255,255,0.03); padding: 16px; border-radius: 16px; margin-bottom: 24px;">
                                <!-- JS populated -->
                            </div>
                            ...
                            <div class="alert alert-info"
                                style="font-size: 13px; background: rgba(59, 130, 246, 0.1); border: 1px solid rgba(59, 130, 246, 0.3); color: #3b82f6; margin-bottom: 24px; border-radius: 12px; padding: 12px 16px; display: flex; align-items: center; gap: 10px;">
                ...
                <?php else: ?>
                    ...
                    <div class="widget" style="padding: 40px; border-radius: 32px; text-align: center;">
                        <div class="success-icon"
                            style="background: <?= $pendingApproval ? '#f59e0b' : '#10b981' ?>20; color: <?= $pendingApproval ? '#f59e0b' : '#10b981' ?>; width: 80px; height: 80px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 32px; margin: 0 auto 24px;">
                            <i class="fa-solid <?= $pendingApproval ? 'fa-clock' : 'fa-check' ?>"></i>
                        </div>
                        <h2 style="font-weight: 800; color: var(--text-main);">
                            <?= $pendingApproval ? 'Pending Approval' : 'Withdrawal Successful!' ?></h2>
                        <p style="color: var(--text-muted); margin-bottom: 32px;">
                            <?= $pendingApproval ? 'Withdrawals over 5,000,000 VND require admin approval.' : 'Your request has been processed successfully.' ?>
                        </p>

                        <div class="summary-card" style="margin-bottom: 32px; text-align: left;">
                            <div class="summary-row"><span
                                    style="color: var(--text-muted);">Amount</span><span><?= number_format($amount, 0, ',', '.') ?>
                                    ₫</span></div>
                            <div class="summary-row"><span style="color: var(--text-muted);">Fee
                                    (5%)</span><span><?= number_format($amount * 0.05, 0, ',', '.') ?> ₫</span></div>
                            <div class="summary-total"><span>Total
                                    Deducted</span><span><?= number_format($amount * 1.05, 0, ',', '.') ?> ₫</span></div>
                        </div>

                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                            <a href="Withdraw.php" class="btn"
                                style="background: var(--bg-body); border: 1px solid var(--border-color); color: var(--text-main); text-decoration: none; text-align: center; padding: 12px; border-radius: 12px; font-weight: 600;">Withdraw
                                More</a>
    ...
    <div class="mobile-bottom-nav">
        ...
        <a href="Transfer.php" class="nav-item"><i
